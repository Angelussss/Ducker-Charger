 ...
 *
 * HOW HARDWARE ENCODER MODE WORKS:
 ...
 *   if (delta > 0) { // rotated right }
 *   if (delta < 0) { // rotated left }
...
/**
 * @brief  HAL_GetTick() value of the last rotation or button edge.
 * @note   Unlike Encoder_GetDelta()/Encoder_IsPressed(), this does NOT
 *         consume anything -- safe for a second caller (e.g. the main
 *         loop's inactivity timer) to poll independently of the UI layer,
 *         which already consumes the delta/press events for navigation.
 * @retval Tick of the last detected activity, 0 if none yet.
 */
