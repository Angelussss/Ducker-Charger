 ...
 *   Counts per detent depend on the encoder variant: the EC11E15244G1
 *   mounted on this board gives 2 counts per detent in TI12 (x4) mode.
 *
...
int8_t Encoder_GetDelta(void)
{
    ...
    /* EC11E15244G1: 15 pulses / 30 detents per revolution, i.e. HALF a
     * quadrature cycle per detent. With TIM3 in TI12 (x4) mode that is
     * exactly 2 counts per detent, so the division is by 2 and lossless.
     * REQUIRES the .ioc change: EncoderMode = TIM_ENCODERMODE_TI12
     * (the original TI1/x2 config yields 1 count per detent and the old
     * /4 swallowed slow rotations entirely). */
