 ...
 *          Implementazione del driver SPI per display ILI9341 / ST7789.
 *
 * STATUS / STATO: functional stub – functions compile and produce correct
 * SPI output; full init sequence to be verified with real hardware.
 * Stub funzionale – le funzioni compilano e producono il corretto output SPI;
 * la sequenza di init completa va verificata con l'hardware reale.
 *
 * DISPLAY INIT SEQUENCE (to verify/extend with real hardware):
 * SEQUENZA DI INIT (da verificare/estendere con l'hardware reale):
 *   1. RST low for 10 ms  --> hardware reset / reset hardware
 *   2. RST high, wait 120 ms --> post-reset delay / attesa post-reset
 ...
 *   5. Display On (0x29)
 *
 * HOW TO SEND A COMMAND / COME MANDARE UN COMANDO:
 *   CS low -> DC low  -> SPI byte -> CS high
 *
 * HOW TO SEND DATA (pixels) / COME MANDARE UN DATO (pixel):
 ...
 * Puntatore all'handle SPI, salvato durante Init(). */
static SPI_HandleTypeDef *_hspi = NULL;

/* =========================================================
 * CONTROL PIN MACROS / MACRO PER I PIN DI CONTROLLO
 *
 * Using macros avoids repeating the HAL_GPIO_WritePin call every time
 * and makes the code easier to read.
 * Usare macro evita di ripetere HAL_GPIO_WritePin ogni volta e rende
...
/* =========================================================
 * PRIVATE FUNCTIONS / FUNZIONI PRIVATE
 * ========================================================= */

/**
 * @brief  Send a single byte as a COMMAND (DC low).
 *         Manda un singolo byte come COMANDO (DC basso).
 */
static void send_cmd(uint8_t cmd)
{
    DISP_CS_LOW();
    DISP_DC_LOW();   /* DC low = command / DC basso = comando */
 ...
 *         Manda un singolo byte come DATO (DC alto).
 */
static void send_data(uint8_t data)
{
    DISP_CS_LOW();
    DISP_DC_HIGH();  /* DC high = data / DC alto = dato */
...
void ILI9341_Init(SPI_HandleTypeDef *hspi_ptr)
{
     ...
     * Reset hardware: porta RST basso per 10ms, poi rilascia. */
    DISP_RST_LOW();
    HAL_Delay(10);
    DISP_RST_HIGH();
    HAL_Delay(120);  /* Wait for the display to come out of reset.
                      * Attendi che il display esca dal reset. */

    send_cmd(0x01);   /* Software reset / Reset software */
    HAL_Delay(150);

    send_cmd(0x11);   /* Sleep Out: exit low-power sleep mode.
                       * Esci dalla modalita' sleep a basso consumo. */
    HAL_Delay(120);

    /* Set pixel format to 16 bits per pixel (RGB565).
     * Imposta il formato pixel a 16 bit per pixel (RGB565). */
    send_cmd(0x3A);
    send_data(0x55);  /* 0x55 = 16 bpp */

    send_cmd(0x29);   /* Display On / Accendi il display */
    HAL_Delay(25);
}
...
void ILI9341_SetWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
     ...
     * Column Address Set (0x2A): definisce l'intervallo orizzontale di pixel. */
    send_cmd(0x2A);
    send_data(x0 >> 8);  send_data(x0 & 0xFF);  /* start column / colonna iniziale */
    send_data(x1 >> 8);  send_data(x1 & 0xFF);  /* end column   / colonna finale   */

    /* Row Address Set (0x2B): defines the vertical pixel range.
     * Row Address Set (0x2B): definisce l'intervallo verticale di pixel. */
    send_cmd(0x2B);
    send_data(y0 >> 8);  send_data(y0 & 0xFF);  /* start row / riga iniziale */
    send_data(y1 >> 8);  send_data(y1 & 0xFF);  /* end row   / riga finale   */

    /* Memory Write (0x2C): subsequent bytes are pixel data.
     * Memory Write (0x2C): i prossimi byte sono dati pixel. */
    send_cmd(0x2C);
}
...
void ILI9341_DrawPixel(uint16_t x, uint16_t y, uint16_t color)
{
     ...
     * Manda il colore RGB565 a 2 byte, byte alto prima (big-endian). */
    send_data(color >> 8);
...
void ILI9341_FillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
     ...
     * Pre-calcola i 2 byte del colore (big-endian). */
    uint8_t hi = color >> 8;
    uint8_t lo = color & 0xFF;

    DISP_CS_LOW();
    DISP_DC_HIGH();  /* All subsequent bytes are pixel data.
                      * Tutti i byte seguenti sono dati pixel. */

    uint32_t total_pixels = (uint32_t)w * h;
    for (uint32_t i = 0; i < total_pixels; i++)
    {
        /* Each pixel = 2 bytes: high byte then low byte.
         * Ogni pixel = 2 byte: byte alto poi byte basso. */
...
void ILI9341_SendPixels(uint16_t *data, uint32_t len)
{
     ...
     * Manda tutto il buffer in un unico trasferimento SPI.
     * len pixel * 2 byte/pixel = len*2 byte totali. */
...
uint16_t ILI9341_RGB(uint8_t r, uint8_t g, uint8_t b)
{
     ...
     * Conversione RGB888 -> RGB565:
     *
     *   R: take the 5 most significant bits (r >> 3), place at bits 11–15.
     *      Prendi i 5 bit piu' significativi (r >> 3), metti in posizione 11–15.
     *   G: take the 6 most significant bits (g >> 2), place at bits 5–10.
     *      Prendi i 6 bit piu' significativi (g >> 2), metti in posizione 5–10.
     *   B: take the 5 most significant bits (b >> 3), place at bits 0–4.
     *      Prendi i 5 bit piu' significativi (b >> 3), metti in posizione 0–4. */
