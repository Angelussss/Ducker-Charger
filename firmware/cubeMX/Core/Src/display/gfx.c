/**
 * @file    gfx.c
 * @brief   pixel tools: font, lines, rects, scaled text, bitmap blit.
 */
...
{
    /* glyph not in font: skip quiet */
    if (c < 32 || c > 122) return;

    uint8_t char_index = (uint8_t)c - 32;
    const uint8_t *char_data = &font->data[char_index * font->char_h];

    /* font data column-major (font5x7 way): one byte = one column,
     * bit 0 = top pixel. old code read it row-major, text came out
     * alien. do not do that again. */
    for (uint8_t col = 0; col < font->char_w; col++)
    {
        uint8_t col_data = char_data[col];
        for (uint8_t row = 0; row < font->char_h; row++)
        {
            uint16_t color = (col_data & (1u << row)) ? fg_color : bg_color;
...
{
    if (c < 32 || c > 122) return;
    const uint8_t *char_data = &font->data[((uint8_t)c - 32) * font->char_h];
                          ...
                          uint16_t fg_color, uint16_t bg_color)
{
    ...
    uint16_t cx = x;
    while (*str) {
        draw_char_scaled(cx, y, *str++, font, scale, fg_color, bg_color);
        cx += (font->char_w + 1) * scale;
    }
}
                    ...
                    const uint16_t *data)
{
    for (uint16_t j = 0; j < h; j++)
        for (uint16_t i2 = 0; i2 < w; i2++)
            ILI9341_DrawPixel(x + i2, y + j, data[j * w + i2]);
}
