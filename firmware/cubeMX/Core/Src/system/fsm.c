...
// ---- Transition matrix ----
// Rows: current state. Columns: event. Value: next state, STATE_NUMBER = no transition.
// GCC range designator [0 ... N] initializes all cells to STATE_NUMBER,
// then specific entries override.
...
static const State_ID_t TransitionTable[STATE_NUMBER][EVT_NUMBER] = {
    ...
    [STATE_SAFETY_LOCK] = {
        [0 ... EVT_NUMBER-1]       = STATE_NUMBER,
        [EVT_CHARGER_CONNECTED]    = STATE_CHARGING, // conditional: see PB_FSM_FireEvent
        [EVT_SOC_LOWV]             = STATE_LOW_V,
        [EVT_SOC_UNDERV]           = STATE_EMERGENCY,
...
void PB_FSM_FireEvent(FSM *fsm, Event_t event) {
    if (event == EVT_SOC_OVCH) fsm->ovchargeBlock = true;
    if (event == EVT_SOC_OK)   fsm->ovchargeBlock = false;

    // Conditional: SAFETY_LOCK → CHARGING is blocked while overcharge is active
    if (fsm->currentState == STATE_SAFETY_LOCK &&
        event == EVT_CHARGER_CONNECTED &&
        fsm->ovchargeBlock)
        return;
...
static void DeepSleep_Enter(FSM *fsm) {
    (void)fsm;
    disable_USBA1(); disable_USBA2(); disable_USBC2(); disable_STPD01();
    HAL_GPIO_WritePin(BCKL_CTRL_GPIO_Port, BCKL_CTRL_Pin, GPIO_PIN_RESET);
    HAL_SuspendTick();
    HAL_PWR_EnterSTOPMode(PWR_LOWPOWERREGULATOR_ON, PWR_STOPENTRY_WFI);
    // Execution resumes here after EXTI wakeup
    HAL_ResumeTick();
    SystemClock_Config();
}
...
static void Manual_Run(FSM *fsm) {
    (void)fsm;
    // Encoder reading and STPD01 voltage/current setup are handled by the UI layer
}
...
static void Error_Enter(FSM *fsm) {
    (void)fsm;
    // The charge layer already detached the faulted subsystem when it pushed EVT_ERROR.
    // No blanket port disable here — only the affected subsystem was shut down.
}
